...
namespace AdventOfCode2021
{
	public class Day4
	{
		...
		{
			var bingoBoards = SetupBingoBoards();
			var drawnNumbers = DrawNumbers();

			//var result = RunPart1(bingoBoards, drawnNumbers);
			var result = RunPart2(bingoBoards, drawnNumbers);

			return result;
		}

		// Get the bingo board that would win last
		private static int RunPart2(List<BingoBoard> bingoBoards, List<int> drawnNumbers)
		{
			foreach (int drawnNumber in drawnNumbers)
			{
				for (int i = 0; i < bingoBoards.Count; i++)
				{
					...
					bingoBoard.MarkValue(drawnNumber);

					if (bingoBoard.HasBingo())
					{
						// If this isn't the last bingo board, remove it from the list
		...
		private static int RunPart1(List<BingoBoard> bingoBoards, List<int> drawnNumbers)
		{
			foreach (int drawnNumber in drawnNumbers)
			{
		...
		private static List<BingoBoard> SetupBingoBoards()
		{
			...
			foreach (var set in sets)
			{
				...
				foreach (var row in rows)
				{
					...
					{
						bingoBoard.Numbers[rowCount, columnCount] = int.Parse(column);
	...
	internal class BingoBoard
	{
		...
		public bool[,] MarkedNumbers { get; set; }
		...
		public bool HasBingo()
		{
			...
			for (int i = 0; i < 5; i++)
			{
				bool rowIsBingo = true;

				// Check each column in the row
				for (int j = 0; j < 5; j++)
				{
					// If this number isn't marked, then no bingo! Move onto the next row
			...
			for (int j = 0; j < 5; j++)
			{
				bool columnIsBingo = true;

				// Check each row in the column
				for (int i = 0; i < 5; i++)
				{
					// If this number isn't marked, then no bingo! Move onto the next column
					if (!MarkedNumbers[i, j])
					{
						columnIsBingo = false;
						break;
		...
		public int GetScore()
		{
			...
			for (int i = 0; i < 5; i++)
			{
				for (int j = 0; j < 5; j++)
				{
					// If this number isn't marked, then add its value to the sum
					if (!MarkedNumbers[i, j])
					{
						sum += Numbers[i, j];
					}
